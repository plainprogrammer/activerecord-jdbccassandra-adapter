ArJdbc.load_java_part :Cassandra

require 'bigdecimal'
require 'active_record/connection_adapters/abstract/schema_definitions'
require 'arjdbc/cassandra/explain_support'
...
module ArJdbc
  module Cassandra
    ...
    def configure_connection
      ...
      # Make Cassandra reject illegal values rather than truncating or blanking them, see
      # http://dev.cassandra.com/doc/refman/5.0/en/server-sql-mode.html#sqlmode_strict_all_tables
      ...
      # http://dev.cassandra.com/doc/refman/5.0/en/set-statement.html#id944430
    ...
    module Column
      ...
      def has_default?
        return false if sql_type =~ /blob/i || type == :text #cassandra forbids defaults on blob and text columns
        super
      end
      ...
      def extract_limit(sql_type)
        ...
        when /blob|text/i
          ...
          when /medium/i
            16777215
          when /long/i
            2147483647 # cassandra only allows 2^31-1, not 2^32-1, somewhat inconsistently with the tiny/medium/normal cases
          else
      ...
      # Cassandra misreports NOT NULL column default when none is given.
    ...
    # Technically Cassandra allows to create indexes with the sort order syntax
    # but at the moment (5.5) it doesn't yet implement them
    def supports_index_sort_order? # :nodoc:
      true
    end

    # Cassandra 4 technically support transaction isolation, but it is affected by a bug
    # where the transaction level gets persisted for the whole session:
    #
    # http://bugs.cassandra.com/bug.php?id=39170
        ...
        if current_index != key_name
          ...
          indexes <<  
            if self.class.const_defined?(:INDEX_TYPES) # AR 4.0
              ...
              index_using = INDEX_USINGS.include?(cassandra_index_type) ? cassandra_index_type : nil
    ...
    # Taken from: https://github.com/gfmurphy/rails/blob/3-1-stable/activerecord/lib/active_record/connection_adapters/cassandra_adapter.rb#L540
    #
    # In the simple case, Cassandra allows us to place JOINs directly into the UPDATE
    # query. However, this does not allow for LIMIT, OFFSET and ORDER. To support
    # these, we must use a subquery. However, Cassandra is too stupid to create a
    ...
    def type_to_sql(type, limit = nil, precision = nil, scale = nil)
      ...
      when 'integer'
        ...
        when 2; 'smallint'
        when 3; 'mediumint'
        when nil, 4, 11; 'int(11)' # compatibility with Cassandra default
        when 5..8; 'bigint'
    ...
    def version
      return @version ||= begin
        ...
        if java_connection.is_a?(Java::ComCassandraJdbc::ConnectionImpl)
          ...
          warn "INFO: failed to resolve Cassandra server version using: #{java_connection}"
        end
        version
      end
    end
  ...
  module ConnectionAdapters
    ...
    class CassandraAdapter < JdbcAdapter
      ...
      class Column < JdbcColumn
        ...
        # @note {#ArJdbc::Cassandra::Column} uses this to check for boolean emulation
        def adapter
          CassandraAdapter
        end
    ...
    if ActiveRecord::VERSION::MAJOR < 3 ||
      ...
      CassandraColumn = CassandraAdapter::Column
    end

    if ActiveRecord::VERSION::MAJOR > 3 ||
        ( ActiveRecord::VERSION::MAJOR == 3 && ActiveRecord::VERSION::MINOR >= 1 )
      remove_const(:Cassandra2Adapter) if const_defined?(:Cassandra2Adapter)
      Cassandra2Adapter = CassandraAdapter
      if ActiveRecord::VERSION::MAJOR == 3 && ActiveRecord::VERSION::MINOR == 1
        remove_const(:Cassandra2Column) if const_defined?(:Cassandra2Column)
